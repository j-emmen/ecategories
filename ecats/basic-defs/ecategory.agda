...
record is-ecategory {ℓ : Level} (Obj : Set ℓ) (Hom : Obj → Obj → setoid) : Set ℓ where
  -- notation
  infix 2 _~_
  _~_ : {a b : Obj} → (f f' : || Hom a b ||) → Set
  f ~ f' = < Hom _ _ > f ~ f'
  infixr 5 _∘_
  field
...
record is-hom-ext (ℂ : ecategory) (Prop : {X Y : ecat.Obj ℂ} → || ecat.Hom ℂ X Y || → Set₁) : Set₁ where
  open ecategory ℂ
  field
