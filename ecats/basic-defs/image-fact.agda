...
module image-fact-defs (ℂ : ecategory) where
  ...
  record img-fact-is-pb-stable {A B C : Obj} {f : || Hom A B ||} (imgf : img-fact-of f) {g : || Hom C B ||}
                               (g×/f : pullback-of g f) (g×/mf : pullback-of g (img-fact-of.M imgf))
                               {pbC : || Hom (×/of.ul g×/f) (×/of.ul g×/mf) ||}
                               (pbtr : ×/of.π/₁ g×/mf ∘ pbC ~ ×/of.π/₁ g×/f) : Set₁
                               where
    private
    ...
    field
      img-pb-stable : is-img-fact {g×/f.ul} {g×/mf.ul} {C} {g×/f.π/₁} {pbC} {g×/mf.π/₁} pbtr
